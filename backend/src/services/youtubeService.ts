...
class YouTubeService {
  ...
  async searchImages(query: string, limit = 20): Promise<SearchImage[]> {
    ...
    try {
      logger.info(
        `🔍 YouTube Service: Searching for "${query}" (limit: ${limit})`,
      );
      ...
      if (results.length < limit) {
        ...
        for (const altQuery of alternativeQueries) {
          ...
          try {
            ...
            const uniqueResults = altResults.filter(
              (newResult) =>
                !results.some(
                  (existingResult) =>
                    existingResult.videoId === newResult.videoId,
                ),
            );
            results.push(...uniqueResults);
          } catch (error) {
            logger.warn(
              `Alternative YouTube search failed for: ${altQuery}`,
              error,
            );
          }
        }
      ...
      logger.info(
        `🎵 YouTube Service: Found ${results.length} results for "${query}"`,
      );
      ...
      if (this.youtubeApiKey && this.youtubeApiKey.trim() !== '') {
        ...
        if (response.data.items && response.data.items.length > 0) {
          return response.data.items.map((item: YouTubeSearchResult) =>
            this.mapYouTubeResult(item),
          );
        }
      } else {
        logger.warn(
          'YouTube API key not found. Using demo thumbnails. Add YOUTUBE_API_KEY to your .env file for real search results.',
        );
      }
  ...
  getHighQualityThumbnail(
    videoId: string,
    quality: 'maxres' | 'standard' | 'high' | 'medium' = 'maxres',
  ): string {
