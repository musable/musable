...
router.get('/album-artwork', authenticateToken, async (req: AuthRequest, res) => {
  try {
...
router.get('/thumbnail/:videoId', authenticateToken, (req: AuthRequest, res) => {
  try {
    ...
    const thumbnailUrl = youtubeService.getHighQualityThumbnail(videoId, requestedQuality);
