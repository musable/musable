    ...
    const roomsWithParticipation = await Promise.all(
      rooms.map(async (room) => {
        if (userId) {
          ...
          const userRole = isParticipant ? await RoomModel.getUserRole(room.id, userId) : null;
          return {
            ...room,
            isParticipant,
            userRole
...
router.get('/code/:code', async (req, res) => {
  try {
    ...
    res.json({
      ...
      code: room.code,
      host: room.host,
      participant_count: await RoomModel.getParticipants(room.id).then(p => p.length),
...
router.post('/:id/queue', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const roomId = parseInt(req.params.id);
...
router.delete('/:id/queue/:queueId', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const roomId = parseInt(req.params.id);
...
router.patch('/:id/participants/:userId/role', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const roomId = parseInt(req.params.id);
    ...
    const isTargetInRoom = await RoomModel.isUserInRoom(roomId, targetUserId);
    if (!isTargetInRoom) {
